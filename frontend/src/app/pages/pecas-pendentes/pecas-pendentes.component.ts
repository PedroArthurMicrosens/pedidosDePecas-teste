...
@Component({
  ...
  templateUrl: './pecas-pendentes.component.html',
  styleUrl: './pecas-pendentes.component.scss'
})
export class PecasPendentesComponent implements OnInit {
  pedidos: Pedido[] = [];
  ...
  getPedidosPendentesFunc(): void {
    this.http.get<Pedido[]>(`${this.apiService.apiUrl}/pedidos/SOLICITADO`)
  ...
  finalizarPedido(){
    ...
    if(this.authService.isTecnico()){
      fetch(`${this.apiService.apiUrl}/pedidos/${pedidoId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify(dadosPedido) // Converte o objeto JavaScript para JSON
    })
    .then(response => {
        if (!response.ok) {
